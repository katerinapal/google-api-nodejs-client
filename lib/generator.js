var mod_Generator = Generator;
import { DefaultTransporter as transporters_DefaultTransporter } from "../lib/transporters";
import ext_swig_swig from "swig";
import ext_jsbeautify from "js-beautify";
...
var beautify = ext_jsbeautify.js_beautify;
...
var templateContents = ext_fs_fs.readFileSync(API_TEMPLATE, { encoding: 'utf8' });
var indexTemplContents = ext_fs_fs.readFileSync(API_INDEX_TEMPLATE, { encoding: 'utf8' });
 ...
 */
buildurl.safe = true;
ext_swig_swig.setFilter('buildurl', buildurl);
ext_swig_swig.setFilter('getAPIs', getAPIs);
ext_swig_swig.setFilter('oneLine', oneLine);
ext_swig_swig.setFilter('getPathParams', getPathParams);
ext_swig_swig.setDefaults({ loader: ext_swig_swig.loaders.fs(__dirname + '/../templates')});
...
Generator.prototype.generateAPI = function(apiDiscoveryUrl, callback) {
  ...
  transporter.request({
    ...
    var contents = beautify(ext_swig_swig.render(templateContents, { locals: resp }), BEAUTIFY_OPTIONS);
    var exportFilename = './apis/' + resp.name + '/' + resp.version + '.js';
    ext_mkdirp_mkdirp(ext_path_path.dirname(exportFilename), function(err) {
      if (err) {
        return handleError(err, callback);
      }
      ext_fs_fs.writeFile(exportFilename, contents, function(err) {
        if (err) {
          return handleError(err, callback);
...
Generator.prototype.generateIndex = function(callback) {
  ...
  transporter.request({
    ...
    var contents = beautify(ext_swig_swig.render(indexTemplContents, { locals: resp }), BEAUTIFY_OPTIONS);
    ext_mkdirp_mkdirp(ext_path_path.dirname(API_INDEX), function(err) {
      if (err) {
        return handleError(err, callback);
      }
      ext_fs_fs.writeFile(API_INDEX, contents, function(err) {
        if (err) {
          return handleError(err, callback);
        }
 ...
 * @this {Generator}
 */
export { mod_Generator as Generator };
