...
import { AuthClient as authclient_AuthClient } from "./authclient.js";
import ext_util_util from "util";
import { PemVerifier as pemverifier_PemVerifier } from "./../pemverifier.js";
import { LoginTicket as loginticket_LoginTicket } from "./loginticket.js";
/**
 ...
 */
ext_util_util.inherits(OAuth2Client, authclient_AuthClient);
...
OAuth2Client.prototype.generateAuthUrl = function(opt_opts) {
  ...
  return rootUrl + '?' + ext_querystring_querystring.stringify(opts);
};
...
OAuth2Client.prototype.revokeToken = function(token, opt_callback) {
  this.transporter.request({
    uri: OAuth2Client.GOOGLE_OAUTH2_REVOKE_URL_ +
      '?' + ext_querystring_querystring.stringify({ token: token }),
    json: true
  }, opt_callback);
};
...
OAuth2Client.prototype.verifySignedJwtWithCerts =
  ...
  var pemVerifier = new pemverifier_PemVerifier();
  ...
  return new loginticket_LoginTicket(envelope, payload);
};
...
export { mod_OAuth2Client as OAuth2Client };
